...
// setPlaceInfoContent(content)),桌面版 7 處、手機版 6 處各自重複這三行,
// 曾經因為漏寫其中一行造成實際 bug(重新搜尋時只清空了 selectedKey,
// PlacePanel 卻沒有跟著關閉,見這裡曾經修過的 commit)。改用判別聯合
...
export type GeoSelection =
  ...
  | { kind: 'info'; key: GeoSelectedKey | undefined; content: PlaceInfoContent }

export type GeoSelectionAction =
  | { type: 'CLEAR' }
  | { type: 'SELECT_ATTRACTION'; key: GeoSelectedKey; data: GeoAttraction }
  | { type: 'SELECT_INFO'; key?: GeoSelectedKey; content: PlaceInfoContent }
  ...
  // 選取狀態誤轉成 info。
  | { type: 'PATCH_INFO_CONTENT'; patch: (content: PlaceInfoContent) => PlaceInfoContent }
...
// onlyIfOutOfView 的完整說明,ExploreMap.tsx 對這個欄位的說明),
