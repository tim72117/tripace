...
struct MembersView: View {
    ...
    /// 目前使用者是否為行程 owner(只有 owner 能改成員權限)。
    private var isOwner: Bool { trip.ownerID == app.currentUser.id }
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingInvite) {
            NavigationStack {
                InviteFriendView(trip: trip, existing: members) { updated in
                    members = updated
                }
            }
        }
    ...
    private func roleControl(for member: Member) -> some View {
        let isTripOwner = member.id == trip.ownerID
        if isTripOwner {
    ...
    private func load() async {
        do { members = try await app.backend.fetchMembers(tripID: trip.id) }
        catch { }
    }

    private func setRole(_ member: Member, _ role: TripRole) async {
        guard member.role != role else { return }
        do { members = try await app.backend.setMemberRole(tripID: trip.id, userID: member.id, role: role) }
        catch { }
    }
}
...
private struct InviteFriendView: View {
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text("輸入已註冊使用者的 Email,即可邀請加入此行程。")
            }
    ...
    private func invite() async {
        ...
        do {
            ...
            let updated = try await app.backend.addMember(tripID: trip.id, email: e, role: .viewer)
            onAdded(updated)
            dismiss()
        } catch {
