...
// 任何特定類型的細節,只查表委派——這與本套件既有的 RegisterTool 模式一致。
//
...
type KindSpec interface {
	...
	ApplyDefaults(args ToolArguments)
}
...
func validateKind(args ToolArguments) error {
...
func applyKindDefaults(args ToolArguments) {
	if s, ok := lookupKind(args.GetString("kind")); ok {
