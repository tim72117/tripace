...
func (p *WantPool) Answer(tripID, question, lang string) model.SearchAnswer {
	return p.For("").Answer(tripID, question, lang)
}

// AssistForSession 依 session 取 analyzer,統一處理 owner 輸入(LLM 自主判斷回答/記錄)。
// 提問時 agent 自己用 query_entries 工具查條目,無需傳入行程訊息。
...
func (p *WantPool) AssistForSession(sessionID, tripID, messageID, text, lang, clientToolsSessionID string, linkMessage func(entryIDs []string) error) AssistResult {
	return p.For(sessionID).Assist(tripID, messageID, text, lang, clientToolsSessionID, linkMessage)
}

// Assistant 是「統一處理 owner 輸入,LLM 自主判斷回答或記錄」的能力。
// 只有 want 引擎(WantPool)實作;規則式分析器不支援(api handler 以斷言判斷)。
type Assistant interface {
	AssistForSession(sessionID, tripID, messageID, text, lang, clientToolsSessionID string, linkMessage func(entryIDs []string) error) AssistResult
}
